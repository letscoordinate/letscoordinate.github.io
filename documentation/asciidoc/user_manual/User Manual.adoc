...
A new notification when arriving in the deck is always an unread notification. User can recognize it by a specific symbol (eye crossed out) on the right part of the reduced view: 
...
* Status of the notifications: Acknowledged/Not acknowledged with this button : User can select to see:
...
By default, only “Not acknowledged” notifications are displayed: As soon as an user will acknowledge a notification, it will disappear based on this filter. 
...
A specific button on the top right is dedicated to hide (or not) the timeline:
...
This functionality is used to change the color of the background of the tool by using a dedicated button close to the logged user:

* **little sun** to switch background to day mode with light colors

image::sun.png[Day mode]

* **little moon** to switch background to night mode with dark colors

image::moon.png[Night mode]
...
link:attached_doc/SERVICE_A_allKpis_allRscs_20200101_20201231.pdf[RSC KPI report - pdf sample]

RSC KPI report - xlsx sample :

link:attached_doc/SERVICE_A_allKpis_allRscs_20200101_20201231.xlsx[RSC KPI report - xlsx sample]
